...
# Google OAuth Config
GOOGLE_CLIENT_ID = "174147128969-2htho9h11jdhqj2inohtu7c316avhjd1.apps.googleusercontent.com"
GOOGLE_CLIENT_SECRET = "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"
...
def index():
    ...
    # If logged in with Google, check that user session has email/name
    if 'user' in session:
        user = session['user']
        if not user.get('email') or not user.get('name'):
            return redirect(url_for('login'))

    # If logged in with normal login, you already have 'username'
