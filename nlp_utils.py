...
def extract_keywords(text, num_keywords=10):
    """
    Extract keywords from text using spaCy and frequency-based analysis,
    with enhancements for handling shorter texts.
    ...
    Returns:
        list: List of dictionaries with keywords and their scores
    """
    try:
        ...
        try:
            ...
            for keyword in candidate_keywords:
                if keyword not in stop_words and len(keyword) > 2:
                    if is_short_text and len(keyword) > 5:
                        keyword_freq[keyword] += 2  
                    else:
...
def extract_topics(text, num_topics=5, num_words=5):
    """
    Extract topics from text using Latent Dirichlet Allocation (LDA).
    ...
    Returns:
        list: List of topics, each containing words and their weights
    """
    try:
        processed_text = preprocess_text(text)
        simple_sentences = []
